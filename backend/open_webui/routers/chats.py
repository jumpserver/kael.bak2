...
@router.post("/{_id}/messages/{message_id}/event", response_model=Optional[bool])
async def send_chat_message_event_by_id(
        _id: str, message_id: str, form_data: EventForm, user=Depends(get_verified_user)
):
...
@router.get("/{_id}/pinned", response_model=Optional[bool])
async def get_pinned_status_by_id(_id: str, user=Depends(get_verified_user)):
    chat = Chats.get_chat_by_id_and_user_id(_id, user.id)
    if chat:
        return chat['pinned']
    else:
...
@router.post("/{_id}/pin", response_model=Optional[ChatResponse])
async def pin_chat_by_id(_id: str, user=Depends(get_verified_user)):
    chat = Chats.get_chat_by_id_and_user_id(_id, user.id)
    if chat:
        chat = Chats.toggle_chat_pinned_by_id(_id)
        return chat
    else:
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED, detail=ERROR_MESSAGES.DEFAULT()
